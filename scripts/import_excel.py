...
def insert_data():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            max_cap = int(row[13]) if not pd.isna(row[13]) else None
            table_cap = int(row[14]) if not pd.isna(row[14]) else None
            naver_link = row[15]
            kakao_link = row[16]

            shop_type_index = row[17]
            shop_type = SHOP_TYPE_MAP.get(shop_type_index, 0) # default = 'meal'

            tag_names = parse_tags(row[18])
